...
function Home() {
  ...
  return <>
    ...
    {latest === undefined ? <Loading /> : latest && <section className="featured"><div><div className="eyebrow accent">THE LATEST ENTRY</div><Link className="feature-link" to={`/posts/${latest.slug}`}><h2>{latest.title}</h2></Link><p>{latest.excerpt}</p><Meta post={latest} /><Link className="read-link inline-flex items-center gap-2" to={`/posts/${latest.slug}`}>Read the story <ArrowUpRight size={17} /></Link></div></section>}
