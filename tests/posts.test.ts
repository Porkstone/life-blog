...
async function setupAuthor() {
  ...
  const userId = await t.run(ctx => ctx.db.insert('users', { name: 'Charlie', githubId: '383633', email: 'private@example.com' }))
...
describe('post publishing authorization', () => {
  test('the OAuth account callback rejects a different GitHub account', async () => {
    ...
    await expect(t.mutation(internal.auth.store, { args: {
      ...
      profile: { githubId: '99999', name: 'Charlie' }, signature: 'test-signature',
  ...
  test('the OAuth account callback stores the allowed immutable GitHub ID', async () => {
    ...
    expect(user).toMatchObject({ githubId: '383633', name: 'Charlie' })
  })
  ...
  test('another signed-in account cannot impersonate Charlie by name or email', async () => {
    const t = convexTest(schema, modules)
    const id = await t.run(ctx => ctx.db.insert('users', { name: 'Charlie', email: 'private@example.com', githubId: '99999' }))
  ...
  test('the authorized author publishes persisted content readable without login', async () => {
    ...
    expect(await author.query(api.posts.viewer)).toEqual({ name: 'Charlie', canPublish: true })
